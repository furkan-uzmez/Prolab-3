...
def shortest_path_collaborators(author,names): # 4.ister
    graph = {author.name : author.dict_edges}
    ...
    for x in unvisited:
        if x not in visited:
            shortest_paths[x],distances[x],_ = dijkstra.shortest_path(graph,author.name,x)
            visited.add(x)
...
def process_ister1(author_a, author_b):
    ...
    return f"Yazar1:{yazar1.name}<br>Yazar2:{yazar2.name}<br><br>En Kısa Yol:<br>{formatted_path}<br><br>Maliyet: {distance}"
...
def process_single_author(author_id, ister_number):
    ...
    if ister_number == 2:
        ...
        return "Yazar - Ağırlık(Makale Sayısı)<br>" + formatted_path
    elif ister_number == 3:
        return f"""
            3. İster Sonuçları:
        ...
        """
        #shortest_paths,distances = shortest_path_collaborators(yazar,names)
        string = " "
        ...
        formatted_path = "<br>".join([f"{bağlantı} : {ağırlık}" for bağlantı, ağırlık in yazar.dict_edges.items()])
        return f"""
                {yazar.name} : {yazar.get_lenght_edges()} <br>
                Bağlantılar :<br>
                {formatted_path}<br>
                """
    elif ister_number == 7:
        ...
        distance = len(path)
        formatted_path = "<br>".join(path)
        return f"{formatted_path}<br>Maliyet: {distance}"
